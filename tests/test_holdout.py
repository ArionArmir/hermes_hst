...
def sigillo(tmp_path, monkeypatch):
    ...
    with open(manifest, "w") as f:
        yaml.safe_dump({"sigillato_il": "2026-07-16", "stato": "SIGILLATO",
                        "aperto_da": None,
                        "simboli": {"ADAUSDT": {"barre": 100}, "LINKUSDT": {"barre": 100}}}, f)
...
def test_ricerca_su_simbolo_sigillato_solleva(sigillo):
    ...
    with pytest.raises(HoldoutViolation, match="SIGILLATI"):
        H.assert_research_allowed(["BTCUSDT", "ADAUSDT"])
...
def test_apertura_registra_ipotesi_e_tentativi(sigillo):
    dati = H.open_seal("soglia 0.50 batte 0.55", n_trials=41, motivazione="predetta dai bucket")
    assert set(dati) == {"ADAUSDT", "LINKUSDT"}
    assert len(dati["ADAUSDT"]) == 100
    man = yaml.safe_load(open(H.MANIFEST_PATH))
    assert man["stato"] == "APERTO"
    assert man["aperto_da"]["tentativi_dichiarati"] == 41
    assert H.count_trials("APERTURA_HOLDOUT") == 1


def test_seconda_apertura_bloccata(sigillo):
    H.open_seal("prima ipotesi", n_trials=41, motivazione="x")
    # Riaprire per una seconda selezione rende l'holdout un set di ricerca:
    # deve costare una scelta esplicita, non capitare per distrazione
    with pytest.raises(HoldoutViolation, match="GIÀ APERTO"):
        H.open_seal("seconda ipotesi", n_trials=3, motivazione="y")


def test_seconda_apertura_consapevole_permessa(sigillo):
    H.open_seal("prima", n_trials=41, motivazione="x")
    dati = H.open_seal("seconda", n_trials=3, motivazione="y", acknowledge_burned=True)
    assert set(dati) == {"ADAUSDT", "LINKUSDT"}
