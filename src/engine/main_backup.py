...
class TradingEngine:
    def __init__(self):
        ...
        # --- MODELLI ---
        # 1. Modello di Uscita (ATR)
        self.exit_model = ATRExitModel(atr_multiplier_sl=2.0, atr_multiplier_tp=3.0)

        # 2. Modello di Conferma (Volume & Pattern)
        self.pattern_model = VolumePatternAnalyzer(window=20)
                    ...
                    while self.running:
                        try:
                            ...
                            if 'data' in data:
                                ...
                                # Aggiorna ultimi prezzi
                                self.latest_prices[symbol] = price

                                # Salva su Redis per dashboard
                                ...
                                # Aggiorna trailing stop dinamico
                        ...
                        if abs(new_sl - pos.stop_loss) / pos.stop_loss > 0.001:
    ...
    async def _open_position(self, signal: Signal):
        ...
        # 1. Pattern Confirmation (se abilitata)
        ...
        # 2. Calcolo SL/TP con ATR (se abilitato)
        ...
        # 3. Side (long/short) – il modello ML decide
    ...
    async def _on_signal(self, signal: Signal):
        ...
        if signal.source == 'ml':
            ...
            if self.reverse_trading_enabled:
                # Se abbiamo una posizione aperta e riceviamo un segnale opposto, chiudiamo la posizione
                if signal.symbol in self.positions and self.positions[signal.symbol].is_open:
                    ...
                    if (signal.action == 'buy' and current_pos.side == 'short') or (signal.action == 'sell' and current_pos.side == 'long'):
                        ...
                        # Attendiamo un momento prima di aprire la nuova posizione
