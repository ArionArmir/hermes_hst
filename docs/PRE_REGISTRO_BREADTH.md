...
| Già toccati (in `data/historical/`, inclusi i nostri 7) | 16 |
| Sigillati (holdout, vietati) | 11 |
| **Disponibili e mai toccati** | **31** |

**Universo massimo: 7 operativi + 31 nuovi = 38 simboli.** Ne servono ~18: il
margine c'è.
...
*Validazione obbligatoria prima del run*: verificare che i 31 nuovi parquet non
abbiano buchi interni (sospensioni di trading). Se l'intersezione risultasse
sensibilmente più corta di 5.5 anni, il run si ferma e il problema si affronta
...
| Universo | Simboli |
|---|---|
| U7 | 7 estratti dai 38 |
| U14 | 14 estratti dai 38 |
| U21 | 21 |
| U28 | 28 |
| **U38** | tutti e 38 — **test primario** |

I sottoinsiemi sono **annidati e casuali con seme fissato (42)**: si mescolano i
38 una volta sola e si prendono i primi `n`. Ordinarli per anzianità
...
**La decisione di promozione si prende solo su U38**, dichiarato ora. Le altre
quattro servono a leggere la curva di scala e **non sono candidati**: se U21
...
- **H5a — L'edge generalizza.** Su U38 la configurazione congelata mantiene
...
- **H5b — La scala segue la Legge Fondamentale.** Lo Sharpe totale cresce
  ~√breadth lungo U7→U38. *Falsificata se:* piatta o decrescente. Una scala
  sublineare è comunque attesa e non falsifica: le cripto sono correlate, e 38
  simboli non sono 38 scommesse indipendenti.
- **H5c — Promuovibile.** U38 supera tutti i criteri sotto.
...
4. **Bootstrap a blocchi mensili: IC 95% che esclude lo zero.** Non è un
   ...
   e con 38 cripto correlate non lo sono — **il DSR sovrastima**. Il bootstrap
...
Se U38 fallisce: **H5-nulla**, holdout non aperto, e la ricerca sui dati
storici è chiusa.
...
Invariato. Lotti A e B sigillati. Apertura solo se U38 supera tutto, come atto
separato e deliberato, su un solo candidato.

Nota: se U38 passasse, il test sull'holdout avrebbe un valore particolare —
...
- [ ] U38 — trade prodotti: __ / 1.344 richiesti
